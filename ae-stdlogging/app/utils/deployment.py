...
def asyncio_run(func: Callable[P, Coroutine[Any, Any, R]]) -> Callable[P, R]:
    """A decorator to run the decorated async function within asyncio.run."""
    ...
    if env_vars_string:
        for pair in env_vars_string.split(","):
            if "=" in pair:
                ...
                logging.warning(
                    f"Skipping malformed environment variable pair: {pair}"
                )
    return env_vars
